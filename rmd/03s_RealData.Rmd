...
* are there outliers (large residuals) or observations with strong influence on fit (large leverage)

"fit" is a large object (list) which contains also lots of diagnostic informmation that can be visualized. Notice how the behaviour of "plot" changes.
```{r, eval=FALSE}
...
**Task**: Repeat the analysis with an added outlier in the data 
```{r}
addrow=iris[1,]             # copy first row
addrow[1:4]=10*addrow[1:4]  # screw values by factor 10
irisout=rbind(iris,addrow)  # add to data frame

# or
irisout=rbind(iris,list(5.8,3, 4, 20, "virginica"))  # make sure to use list
```
