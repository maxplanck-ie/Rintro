...
peaks<-fread("/data/processing/sikora/GSE77784_MACS2.workspaceR/bam1.annotated.sub.txt",header=TRUE,sep="\t")
dge_res$HasPeak<-"No"
...
#z<-table(dge_filt$Status,dge_filt$HasPeak)
#fisher.test(z)
...
dge_filt100<-data.table(rbind(dge_filtBound[sample(nrow(dge_filtBound),100,replace=TRUE),],dge_filtNotBound[sample(nrow(dge_filtNotBound),100,replace=TRUE),]))
boxplot(log2FoldChange~HasPeak,data=dge_filt100)

dge_filt100_1<-data.table(rbind(dge_filtBound[sample(nrow(dge_filtBound),100,replace=TRUE),],dge_filtNotBound[sample(nrow(dge_filtNotBound),100,replace=TRUE),]))
dge_filt100_2<-data.table(rbind(dge_filtBound[sample(nrow(dge_filtBound),100,replace=TRUE),],dge_filtNotBound[sample(nrow(dge_filtNotBound),100,replace=TRUE),]))
...
Advanced reading: https://www.statmethods.net/advstats/bootstrapping.html .
...
```
Important functions: apply().


##TODOs:
+ update paths from local to urls
+ correct header size
