...
**Task**: Repeat the analysis with an outlier in the data 
```{r}
irisout=rbind(iris,c(5.8,3, 4, 20, "virginica"))
```
...
# Logistic regression
Goal: predict species based on on features
```{r}
plot(Species=="setosa" ~ Sepal.Length, data=iris)
model=glm(Species=="setosa" ~ Sepal.Length, family=binomial,data=iris)
points(iris$Sepal.Length,fitted(model),col="red", pch=20)
```
...
par(mfrow=c(1,2))
plot(setosa ~ Sepal.Length, data=x)
plot(setosa ~ Petal.Length, data=x, main="Prefect Separation")
par(op)
plot(x, col=x$setosa+1)
