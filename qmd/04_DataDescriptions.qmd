...
hist() can have many arguments. Use help to find out how the histogram can be customized (e.g. number of break points, title, colors). Try some of it.
...
The boxplot above is for *all* data. Create a boxplot of petal length for species "setosa" only.
...
Use help to add three different colors:
...
remove the Species variable from "iris" and store the result in a new data.frame "niris"
```{r niris, echo=FALSE}
